...
async function fetchQuestions() {
  try {
    const response = await fetch("http://localhost:3000/api/leetcode/sync-all");
...
async function insertQuestions(questions) {
  ...
  const formattedQuestions = questions.map((q) => ({
    platform_id: 1,
    external_question_id: "LC_" + q.problemId,
    question_title: q.title,
    difficulty: q.rating ?? "NA",
    problem_url: q.url,
  }));
...
async function insertQuestionTopics(questions, insertedQuestions, topicMap) {
  ...
  for (const q of questions) {
    const externalId = "LC_" + q.problemId;
